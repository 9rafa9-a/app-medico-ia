...
def main(page: ft.Page):
    try:
        ...
        loading_text = ft.Text("Iniciando sistema...", size=20, color="blue")
        ...
        # Check if attribute exists to avoid crash if API differs slightly
        try:
            encoder = getattr(ft.AudioEncoder, "WAV", "wav") 
            ...
            page.add(ft.Text(f"Erro ao inicializar audio: {e}", color="red"))
        ...
        page.clean()
        page.add(ft.Text(f"Erro CRÍTICO na inicialização: {e}", color="red", size=20))
        page.update()
